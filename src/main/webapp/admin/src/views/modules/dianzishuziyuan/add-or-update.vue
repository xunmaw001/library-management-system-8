<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="电子书" prop="dianzishu">
          <file-upload
          tip="点击上传电子书"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.dianzishu?ruleForm.dianzishu:''"
          @change="dianzishuUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.dianzishu" label="电子书" prop="dianzishu">
            <el-button type="text" size="small" @click="download(ruleForm.dianzishu)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tushumingcheng = true;
          ...
          if(o=='dianzishu'){
            this.ruleForm.dianzishu = obj[o];
	    this.ro.dianzishu = true;
            continue;
          }
          if(o=='tushuzuozhe'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dianzishuziyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.dianzishuziyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    dianzishuUploadChange(fileUrls) {
	this.ruleForm.dianzishu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
